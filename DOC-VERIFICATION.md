...
| README:142-150 | sshd declares a service → charly injects the destination init; box plan checks only composition | C6/C3 | layer skill; tutorial-shell description | read | VERIFIED | `distro-fedora/box/tutorial-shell/charly.yml:8-38` |
...
| D35 | `.pi/README.md:44` | "Claude Code … wire `.claude/hooks/*.sh`" — charly's own `.claude/settings.json` has no hooks block |
...
All nine PRs validated and merged. The `group` provider’s retirement (from `hand`→generated surfaces) is a separate product cutover named `feat/retire-group-kind`; this audit kept `group` wherever the regenerated `providers.md` still listed it.
