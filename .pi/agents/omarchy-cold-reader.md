...
2. **PROCESS** (about the eval itself): class/venue right (wrong tier = REDO-PROCESS)? every PR-specific check known-red (control observed = pass)? media non-empty AND showing the commands? timing within the oracle's budget (drift = anomaly)? Assisted-by footer, template-conform? Any defect → REDO-PROCESS with findings.
