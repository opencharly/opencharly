...
description: |
  ...
  state machine TRIAGE → PLAN → CONTROL → EVAL → EVIDENCE → COLD-READ →
...
## Ownership
- **R0 skills loading (binding)**: every lane run STARTS by loading the repo skills from the eval-omarchy checkout — `candy/eval-pr/charly.yml` (the entry `omarchy-eval` entity, carrying the pr-validator APPLICABILITY MAP) plus the skill each lane touches (oracle: `omarchy-eval-oracle`; cold-reader: `omarchy-eval-cold-reader`). A run that skips them is a contract violation; the lane contract is the `eval-pr-plan` pipeline in `charly.yml` (the `candy/eval-lane/` v1 corpus was consolidated into `candy/eval-pr/`).
...
- **REDO state machine**: TRIAGE → PLAN → CONTROL → EVAL → EVIDENCE → COLD-READ → ACCEPT/REPORT. On REDO-PROCESS: R1 → setup update (in the owning repo) → FULL re-run of that PR. Loop guard: ≥3 REDO entrances for one PR = escalate, never silently re-run. A finding closes only with a setup update + rerun evidence.
- **Measurement** (native, no scripts): read each run's `.check/<bed>/<calver>/summary.yml` (the per-phase durations ARE the ledger) and the committed `eval/pr-<N>/eval.yml` record (written by the `eval-pr-plan` pipeline — there is no separate `metrics/` tree): avg sec/eval, **evals/min** (= lanes × 60 / avg_eval_seconds), per-phase medians, slot-busy % (from the lane board), host load/RAM (read inline when a batch runs), lock errors (grep the run logs for `database is locked` / `Failed to get shared`). The operator target: **≤60 s average per eval** (≈16 evals/min at 16 lanes). Hill-climb one variable per batch; plateau = 3 consecutive batches no improvement AND ≥95% slot utilization. Drift (revert creep, lock reappearance, update-phase regrowth) → fix, not shrug.
...
- **The harsh critical loop**: every stage's work is graded by the agent before/after it (runner grades the oracle via the CONFIG AUDIT; cold-reader grades runner+oracle via PROCESS; oracle re-authors on redo-plan). Every stage can TRIGGER a change: redo-plan / redo-run / redo-read / escalate — contract: the `eval-pr-plan` pipeline's `redo:` edges + the `validate`/`cold-read` stages; the org pr-validator PASS/BLOCK is the `validate.verdict` field, separate from the packet cold-read verdict.
