...
## Ownership
- **R0 skills loading (binding)**: every lane run STARTS by loading the repo skills from the eval-omarchy checkout — `candy/eval-lane/charly.yml (the omarchy-eval entity)` (the entry) plus the skills each lane touches (oracle: omarchy-eval-oracle + tiers; runner: golden + media + sequencing; cold-reader: omarchy-eval-cold-reader + work-lane; loop: omarchy-eval-full-loop). A run that skips them is a contract violation; the CONFIG AUDIT verifies the loaded contract matches the bed.
