...
4. **EVIDENCE**: assemble the artifacts into `media/<pr>-<calver>/` with pi file tools (the record:/spice: steps already pulled `.cast`/`.gif`/frames onto the host; copy them into the media dir), verify both lanes non-empty (rule 6).
