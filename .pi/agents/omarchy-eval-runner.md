...
2. **ORPHAN DISCIPLINE** (measured requirement): a control-FAIL bed leaves its domain running, holding a qemu-img write-lock on the shared clone disk — the next vm-build fails. Between control and eval (and after any failed run): `charly vm stop <bed> --domain <bed> --force` then `charly vm destroy <bed> --domain <bed>`.
...
4. **FULL EVIDENCE EVALUATION**: evaluate ALL available resources — the PR code/diff (do the checks exercise the real behavior, not token presence?), the check results (`eval/pr-<N>/eval.yml` + `.check/<bed>/<calver>/summary.yml` + per-step logs, deterministic truth), the screencast (.cast text: exact commands + timestamps), and the media (frames/video) — and verify both recording lanes non-empty (rule 6). The `eval-pr-plan` pipeline's `media`/`record` stages assemble `eval/pr-<N>/media/` and render the record; the runner does not hand-write a `metrics/` file.
...
Findings report trigger: redo-plan (contract: the `eval-pr-plan` pipeline's `redo:` edges + the entry `omarchy-eval` skill, eval-omarchy `candy/eval-pr/charly.yml`);
the control-FAIL finding is the FAKE-ASSERTION case of this audit (the `control` stage's `setup-defect` redo).
