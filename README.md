...
| `marketplace/` | [opencharly/marketplace](https://github.com/opencharly/marketplace) | the plugin/skill corpus + per-harness catalogs — standalone (pins charly in its own `.gitmodules`; its deploy workflow owns generation); each harness loads it natively |
...
3. Everything else (`sdk`, `spec`, `marketplace`, `docs`, `charly-*`, `pkg-*`,
